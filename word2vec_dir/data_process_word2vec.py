...
class Word2vec():
    ...
    def selectWords(self,docs):
        ...
        print(nltk.vocab())
        select_words = [f[0] for f in nltk_texts.vocab().most_common(self.max_word)]
        np.save("/home/ailsb/PycharmProjects/test/word2vec/data/select_word.npy", select_words)
    ...
    def Convert2Vec(self):  ## Convert corpus into vectors
        print("start convert2vec")

        model = Word2Vec.load('./data/Word2vec_review.model')
    ...
    def load_model(self,model_path):
        model = Word2Vec.load('./data/Word2vec_review2.model')
        return model
...
if __name__ == '__main__':
    ...
    w2v = Word2vec(100, 6500, 100)
    tar,text=w2v.load_data(fns_train)
    train_docs = w2v.data_processing('/home/ailsb/PycharmProjects/test/word2vec/data',tar,text)
    select_words = w2v.selectWords(train_docs)
